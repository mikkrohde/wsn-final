  ...
  const linkaddr_t *src, const linkaddr_t *dest)
{
  ...
  {
    unsigned char parity_bit[len];
    memcpy(&parity_bit, data, sizeof(parity_bit));
    LOG_INFO("Recieved parity bit (%s) from ", parity_bit);
    LOG_INFO_LLADDR(src);
    LOG_INFO_("\n");
  }
...
PROCESS_THREAD(nullnet_example_process, ev, data)
{
  ...
  static unsigned char parity_bit[] = "0x1";
  ...
  while(1) {
    ...
    PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&periodic_timer));
    LOG_INFO("Sending parity bit (%s) to ", parity_bit);
    //LOG_INFO_LLADDR(NULL);
    LOG_INFO_("\n");
    
    nullnet_buf = (unsigned char *)&parity_bit;
